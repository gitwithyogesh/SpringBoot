...
async function loadProductsFromAPI() {
    try {
        ...
        if (response.ok) {
            ...
            console.error('❌ Failed to load products from API - Status:', response.status);
    ...
    if (placeOrderBtn) {
        placeOrderBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const addressResponse = await fetch(`/api/addresses/add?userId=${userId}`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                ...
                if (addressResponse.ok) {
                    ...
                    const existingAddressesResponse = await fetch(`/api/addresses/user/${userId}`);
                    if (existingAddressesResponse.ok) {
                        const addresses = await existingAddressesResponse.json();
                        if (addresses && addresses.length > 0) {
                            addressId = addresses[0].id;
                        } else {
                            throw new Error('Failed to create/get address');
                        }
                    } else {
                        throw new Error('Failed to create/get address');
                    }
                ...
                const orderPromises = cart.map(async (item) => {
                    ...
                    const response = await fetch(`/api/orders/place?userId=${userId}&productId=${item.id}&quantity=${item.quantity}&addressId=${addressId}`, {
                        method: 'POST'
                    });
                ...
                // Store cart count before clearing
                const orderCount = cart.length;

                // Clear cart from localStorage
                localStorage.removeItem('wearify-cart');
                cart.length = 0; // Clear the array without reassigning

                // Show success message
                showNotification(`🎉 Order placed successfully! ${orderCount} items ordered. Thank you!`);
